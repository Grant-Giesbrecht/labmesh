...
class DirectoryBroker:
    ...
    def __init__(self, *, rpc_bind: str = RPC_BIND, xsub_bind: str = XSUB_BIND, xpub_bind: str = XPUB_BIND):
        ...
        self.drivers: Dict[str, Dict[str, Any]] = {}   # service -> {rpc_endpoint: str}
        self.banks: Dict[str, Dict[str, Any]] = {}     # bank_id -> {ingest, retrieve}

    async def _run_state_proxy(self):
        xsub = self.ctx.socket(zmq.XSUB); _curve_server_setup(xsub)
        xsub.sndhwm = 2000; xsub.rcvhwm = 2000; xsub.bind(self.xsub_bind)

        xpub = self.ctx.socket(zmq.XPUB); _curve_server_setup(xpub)
        xpub.sndhwm = 2000; xpub.rcvhwm = 2000; xpub.setsockopt(zmq.XPUB_VERBOSE, 1); xpub.bind(self.xpub_bind)

        self._xsub, self._xpub = xsub, xpub
        print(f"[broker] state proxy XSUB={self.xsub_bind} <-> XPUB={self.xpub_bind}")
        try:
            await zmq.asyncio.proxy(xsub, xpub)
        finally:
            xsub.close(0); xpub.close(0)
    ...
    async def _run_directory(self):
        ...
        while True:
            ...
            if t == "hello":
                role = msg.get("role")
                if role == "driver":
                    svc, ep = msg.get("service"), msg.get("rpc_endpoint")
                    if not svc or not ep:
                        await r.send_multipart([ident, dumps({"type":"error","error":{"code":400,"message":"missing service/rpc_endpoint"}})]); continue
                    self.drivers[svc] = {"rpc_endpoint": ep}
                    await r.send_multipart([ident, dumps({"type":"hello","ok":True,"role":"driver"})])
                    print(f"[broker] driver up: {svc} -> {ep}")
                elif role == "bank":
            ...
            if t == "rpc":
                ...
                if method == "list_services":
                    result = [{"service": s, **info} for s, info in sorted(self.drivers.items())]
